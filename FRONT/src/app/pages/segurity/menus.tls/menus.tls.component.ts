...
import { SetModulePayload, SetMainMenuPayload, SetSubMenuPayload, StateOption, MenuOption, City, MenuGeneralForm, MenuPortfolioForm, SubMenuForm, Step1Form, Step2Form, Step3Form, ModuleItem, MainMenuItem, SubMenuItem} from '../../../interfaces/tools/menu/menu.interface';
...
export class MenusTlsComponent implements OnInit {
  ...
  saveSubMenu(isDeleted: number = 1): void {
    this.formSubmitted = true;
    if (!this.subMenuForm.shortDescription.trim() || !this.subMenuForm.longDescription.trim() || !this.subMenuForm.route.trim() || !this.subMenuForm.order || !this.subMenuForm.mainMenuId) {
      this._messageService.add({ severity: 'warn', summary: 'Warning', detail: 'All fields are required' });
      return;
    }
  ...
  private resetSubMenuForm(): void {
    this.subMenuForm = { shortDescription: '', longDescription: '', route: '', order: null, mainMenuId: null };
  }
