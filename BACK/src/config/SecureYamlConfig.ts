...
export class SecureYamlConfig implements ConfigPort {
  ...
  get ENCRYPTION_KEY(): string {
    return this.decrypt(this.configData.ENCRYPTION_KEY);
  }
  ...
  get STORAGE_KEY(): string {
    return this.decrypt(this.configData.STORAGE_KEY);
  }

  get AZURE_STORAGE_CONNECTION_STRING(): string {
    return this.decrypt(this.configData.AZURE_STORAGE_CONNECTION_STRING);
  }
