...
export class OriginationCatalogController {
  ...
  private getMilestonesTDD = async (res: Response, next: NextFunction) => {
    try {
      const result = await ServiceContainer.catalogos.getMilestonesTDD.run();
      return res.status(200).json({ valido: 1, result });
    } catch (error) {
      next(error);
    }
  ...
  private getSmeArea = async (res: Response, next: NextFunction) => {
    try {
      const result = await ServiceContainer.catalogos.getSmeByArea.run();
      return res.status(200).json({ valido: 1, result });
    } catch (error) {
      next(error);
    }
  ...
  cat_municipalities_by_state = async (req: Request, res: Response, next: NextFunction) => {
    ...
    return this.getMunicipalitiesByState(id, res, next);
  }

  cat_agrarian_nucleus_by_municipality = async (req: Request, res: Response, next: NextFunction) => {
    const id = Number(req.params.id as string)
    return this.getAgrarianNucleusByMunicipality(id, res, next);
  }

  ct_status_document = async (req: Request, res: Response, next: NextFunction) => {
    return this.getCatalog('ct_status_document', res, next);
  }
  ...
  ct_buffer_pool = async (req: Request, res: Response, next: NextFunction) => {
    return this.getCatalog('ct_buffer_pool', res, next);
  }

  ct_inventory_stratification = async (req: Request, res: Response, next: NextFunction) => {
    return this.getCatalog('ct_inventory_stratification', res, next);
  }

  ct_grievance_mechanism = async (req: Request, res: Response, next: NextFunction) => {
    return this.getCatalog('ct_grievance_mechanism', res, next);
  }

  ct_press_negative = async (req: Request, res: Response, next: NextFunction) => {
    return this.getCatalog('ct_press_negative', res, next);
  }
  ...
  cat_milestones_tdd = async (req: Request, res: Response, next: NextFunction) => {
    return this.getMilestonesTDD(res, next);
  }

  cat_smes = async (req: Request, res: Response, next: NextFunction) => {
    return this.getSmeArea(res, next);
  }
